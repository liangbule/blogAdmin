...
class BaseInfo extends React.Component<IFormComponentProps<any>, any> {
    state = {
        allTags: [],
        tagIds: [],
                ...
                <Form layout="inline"
                    ...
                    <Col span={14}>
                        <Form.Item label="标签" rules={[{required: true, message: '请选择标签!'}]}>
                            ...
                            >
                                {
                                    allTags.map((item: any) => <Option key={item._id}
                    ...
                    <Col span={10}>
                        <Form.Item label="是否发布" rules={[{required: true, message: '是否现在要发布!'}]}>
                            <Switch checkedChildren="发布" unCheckedChildren="草稿" checked={article.state === 1}
                                    onChange={(checked: boolean) => this.switchChange('state', checked)}/>
                        </Form.Item>
                    </Col>
                    <Col span={14}>
                        <Form.Item label="是否公开" rules={[{required: true, message: '是否是公开文章!'}]}>
                            <Switch checkedChildren="公开" unCheckedChildren="私密" checked={article.publish === 1}
